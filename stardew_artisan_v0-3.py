...
def initialize_window():
    root.title("Stardew Artisan v 0.3")
    ...
    for i in order:
        ...
        if  season in cropData[i][5] or season == "All":
            if (qualityOpt == "yes" and cropData[i][8] == "yes" and process == "Keg") or qualityOpt == "no":
                ...
                if qualityOpt == "yes":
                    cost = calculate_cost(quality, cropData[i][cost])
                    cropCost = Label(frames[3], text="%d g" % cost, anchor = W)
                else:
                    cropCost = Label(frames[3], text="%d g" % int(cropData[i][cost]), anchor = W)
                cropCost.grid(row = row, column = 1, sticky = W)

def calculate_cost(quality, normalCost):
    cost = int(normalCost.strip("g"))
